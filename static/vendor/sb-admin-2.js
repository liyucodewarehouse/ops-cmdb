...
var RebootPage = function(opt) {
    ...
    // 是否显示添加按钮
    this.showAddBtn = opt.showAddBtn || 'yes'
    // 是否显示更新按钮
    this.showUpdateBtn = opt.showUpdateBtn || 'yes'
    // 是否显示更新删除
    this.showDeleteBtn = opt.showDeleteBtn || 'yes'
    // 是否有自定义按钮，在表格内部
    this.customizedButton = opt.customizedButton
    // 是否有自定义按钮，在表格头部
    this.customizedButtonAtHeader = opt.customizedButtonAtHeader
    // 预加载数据，预加载完毕之后，才渲染页面
    this.preload = {}
    //弹出窗的标签
...
$.extend(RebootPage.prototype, {
    ...
    initForm: function() {
        ...
        $.each(this.data, function(indev, val) {
            ...
            //是否允许手动修改
            val.manualChage = val.manualChage || 'yes'
            if (val.manualChage == 'yes') {
    ...
    // 添加一些按钮在表格上面，如"添加表单"、"导出Excel"等
    initAddSomeBtnAtHeader: function() {
    	// 新增一个"添加表单"的按钮在表格上面
    	addBtnHTML = ''
    	if (this.showAddBtn == "yes"){
        	addBtnHTML = '<button type="button" class="btn btn-primary btn-sm add-modal"' +
            ' data-target="#add' + this.name + 'modal">' +
            '添加' + this.title + '</button>  '
        }
    	// 新增一些自定义按钮在表格头部
    	customizedButtonAtHeaderHTML = ''
    	if (this.customizedButtonAtHeader){
            $.each(this.customizedButtonAtHeader, function(key, value) {
                customizedButtonAtHeaderHTML += '<a href="' + value.url + '" target="_blank"><button type="button" class="btn btn-primary btn-sm">' + value.text + '</button></a>'
            });
        }
    	// 新增一个"导出Excel"的按钮表格上面
	    exportBtnHTML = '   <a href="/export/excel?action_type=' + this.name + '" target="_blank">' +
            '<button type="button" class="btn btn-primary btn-sm">导出Excel</button></a>  '
        $('#main-content').prepend('<p style="position: absolute;z-index:200">' + addBtnHTML + customizedButtonAtHeaderHTML + exportBtnHTML + '</p>')
    ...
    bindEvents: function() {
        ...
        $(document).off('click.fetchOneNetworkConfig').on('click.fetchOneNetworkConfig','.fetchOneNetworkConfig',function() {
            var id = $(this).data('id')
            $.jajax('/fetchOneNetworkConfig/' + id,function(res){
                swal("备份成功!", '', 'success');
            })
        })
    },
    ...
    getlist: function() {
        ...
        $.jajax('/listapi?action_type=' + that.name, function(data) {
            ...
            $.each(data.result, function(i, v) {
                var btn = ['<button style="margin:5px 5px 0px;" class="btn btn-xs btn-primary" ']
                arr.push('<tr>')
                ...
                $.each(that.data, function(index, value) {
                    ...
                    if (!value.hide) {
                        ...
                        arr.push('<td>' + name + '</td>')
                			...
                			//alert(btn.join(''))
                			//BUG - 大写变小写了
                			//alert($(btn.join('')).html("hello").prop('outerHTML'))
                var operateBtn = ''
                //自定义按钮，在表格内部
                if(that.customizedButton){
                	$.each(that.customizedButton, function(key, value) {
                		if(value.name == "fetchOneNetworkConfig"){
                			//网络配置管理，备份按钮
                			operateBtn += $(btn.join('')).addClass('fetchOneNetworkConfig').html(value.text).prop('outerHTML')
                		}else{
                			operateBtn += '<a href="' + value.url + v.id +'" target="_blank">' + $(btn.join('')).html(value.text).prop('outerHTML') + '</a>'
                            
                		}
                	});
                }
                //更新、详细、删除 按钮
                if(that.showUpdateBtn == "yes"){
                    operateBtn += $(btn.join('')).addClass('update').html('更新').prop('outerHTML')
                }
                if (that.modal_detail) {
                    operateBtn += $(btn.join('')).addClass('detail').html('详细').prop('outerHTML')
                };
                if(that.showDeleteBtn == "yes"){
    ...
    renderDatatable: function(str) {
        ...
        //排序
        var sortAt2ndField_desc=new Array("NetworkConfigManagement","HostDailyCheck")
        if( $.inArray(this.name,sortAt2ndField_desc) >= 0){
        	//第二列倒排序
            $('.table').DataTable({
                responsive: true,
                bLengthChange: true,
